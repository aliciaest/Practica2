...
grafico1
grafico2<-ggplot(ds, aes(x = MinTemp, y = MaxTemp, colour=RainTomorrow))
grafico2<-grafico1+geom_point()
grafico2
grafico3<-grafico2+ggtitle("Grafico de dispersion")
grafico3
...
grafico<- ggplot(ds, aes(x= Temp9am, y=Pressure9am, shape=RainToday))
grafico2<-grafico+geom_point()

grafico3<-ggplot(ds, aes(x= Temp9am, y=Pressure9am, shape=RainToday, colour=RainToday))
grafico4<-grafico3+ geom_point()
grafico4
grafico4<-grafico4+ggtitle("Grafico de dispersion del ejercicio 2")
...
str(iris.tidy)
#asi podemos ver que el fichero contiene 4 variables : species, part, measure y value
...
iris.grafico2+geom_point()+facet_grid(.~Measure)

iris.grafico2+geom_jitter()+facet_grid(.~Measure)
#la diferencia es que en este caso los puntos estan mas espaciados y podemos interpretar mejor la informacion
...
str(DatosEjer4)
#nos encontramos con tres variables que son fecha, fabricante y ventas

grafico<-ggplot(DatosEjer4, aes(x=Fecha, y = Ventas, colour= Fabricante))
grafico+geom_point()
#Diriamos que no es valido porque al estar los puntos dispersos no somos capaces de ver con claridad la evolucion

grafico2<- grafico+geom_point()+geom_line(linetype="dashed")
#asi cambiamos el tipo de linea

grafico2+ggtitle("Ventas mensuales de diferentes compañías tecnológicas")
...
str(Experimento)
#podemos ver que hay 4 variables que son Complemento,Dosis, Altura y Error_Altura

grafico<-ggplot(Experimento, aes(x=Dosis, y=Altura, shape=Complemento, linetype=Complemento, colour=Complemento))
grafico+geom_point()+geom_line()

grafico+geom_point()+ geom_line() + geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=.1, position = position_dodge(0.05))
#es asi como añadimos el error
  
grafico+ geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=.1, position = position_dodge(0.05))
...
grafico<- ggplot(ds, aes(x=Year, y =MinTemp))
grafico+geom_boxplot()

grafico2<- ggplot(ds, aes(x=Year, y =MaxTemp))
grafico2+geom_violin() +ggtitle("Distribución de temperaturas máximas por año")
#como no he utilizado colores no hay ninguna leyenda que eliminar
...
f<-read.table(url("http://assets.datacamp.com/blog_assets/chol.txt"), header=TRUE)
str(f)
grafico<- ggplot(f, aes(x=HEIGHT)) + geom_histogram()
grafico+labs(title="Histograma de la variable de la altura", x="Altura(cm)", y ="Frecuencia")

grafico2<-ggplot(f,aes(x=HEIGHT) ) + geom_histogram(breaks=seq(155,195,by=2), bins=20)+ labs(title = "Histograma de la variable de la altura", x="Altura(cm)",y ="Frecuencia")
grafico2

grafico3<-ggplot(f,aes(x=HEIGHT) ) + geom_histogram(breaks=seq(155,195,by=2), bins=20, fill="Blue", colour="Red",alpha=0.5)+ labs(title = "Histograma de la variable de la altura", x="Altura(cm)",y ="Frecuencia")
grafico3

grafico4<-grafico3+geom_density(col="Green")
grafico4
